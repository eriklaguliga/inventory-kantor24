...
if(isset($_POST['btnSimpan'])){
	...
	if (count($pesanError)>=1 ){
		...
		if (empty($_FILES['txtNamaFile']['tmp_name'])) {
			...
			if(file_exists("foto/petugas/".$txtFileSembunyi)) {
				unlink("foto/petugas/".$txtFileSembunyi);	
			}
			...
			copy($_FILES['txtNamaFile']['tmp_name'],"foto/petugas/".$nama_file);					
		}
		
		# SIMPAN DATA KE DATABASE (Jika tidak menemukan error, simpan data ke database)
		$Kode	= $_POST['txtKode'];
		$mySql  = "UPDATE petugas SET nm_petugas='$txtNama', kelamin='$cmbKelamin', alamat='$txtAlamat', no_telepon='$txtTelepon', 
					foto='$nama_file', username='$txtUsername', level='$cmbLevel'
					$sqlPasword  
...
# TRANSAKSI PEMBELIAN
	...
	$dataPemeliharaan 		= isset($_POST['rbPemeliharaan']) ? $_POST['rbPemeliharaan'] : $my2Data['mu_trans_pemeliharaan'];
